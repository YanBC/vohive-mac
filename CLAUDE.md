...
- **`server.go`** — HTTP API (`/api/status`, `/api/traffic`, `/api/sms/inbox`, `/api/sms/send`) + embedded `static/` UI. Status is cached 5 s to throttle AT-port polling; the inbox handler triggers `SyncIfStale(30s)` before reading the DB.
